...
GPT-4やClaude APIを使えば、確かに高品質なコードが生成できる。しかし、毎月の請求書を見るたびに思った——「自分のGPUで動く、自分だけのコーディング相棒が欲しい」。

手元にあるのはRTX 3060（12GB）とRTX 4060（8GB）。この「一般家庭のGPU」で動くLLMを作れないか。

### なぜJuliaなのか

Juliaは科学技術計算の言語だ。Pythonのような書きやすさと、C並みの速度を両立する。そして何より、**多重ディスパッチ**という独特の型システムを持つ。
