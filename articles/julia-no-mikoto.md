...
L3（推論）とL5（評価）の物理的隔離によるハルシネーションの構造的封じ込めと、Juliaの型システムを活用したゼロショット多重ディスパッチ解決。──これは、神話の皮を被った「計算機科学の聖域」の構築記録である。

### なぜJuliaなのか

Juliaは科学技術計算の言語だ。Pythonのような書きやすさと、C並みの速度を両立する。そして何より、多重ディスパッチという独自の型システムを持つ。
既存の巨大LLMは、情報の海を漂うだけの「写経機」に過ぎない。しかし、Juliaの魂である型システムを理解し、「黄泉比良坂」という名の物理的ファイアウォールで推論と評価を峻別する知能は、ハルシネーションという「穢れ」を構造的に拒絶できる。
この「型を識る知能」は、Rustの峻烈な型世界をも攻略する可能性を秘めている。巨大な計算リソースで殴り合う時代を終わらせ、知能の「質」と「規律」で世界を再定義する。──最後の一ピースが、「優れたローカルLLM」だ。
